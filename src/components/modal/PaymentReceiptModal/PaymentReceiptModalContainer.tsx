...
import { receivePayment, PaymentReceiveData, deletePayment, updatePayment, PaymentUpdateData, sendEmail } from "@/lib/api/legacyApiAdapter";
import {
  normalizeAmount,
...
import type { EmailFormData } from "@/components/EmailModal/types";
import type {
  PaymentMethod,
  AllocationRow,
  EditLessonRow,
...
/**
 * Global Payment Receipt Modal Container Component
 * Reusable component for displaying payment receipts
 * Used in enrollment pages and future modules
 */
export function PaymentReceiptModalContainer(
  ...
  }, [location, paymentId, mode, directPaymentData, customerId, fetchLocationDetails, locationDetails]);
  ...
  // Email handler - opens email modal
  const handleEmail = React.useCallback(() => {
    const emailContent = generatePaymentReceiptEmail(paymentReceiptData);
    
    // Set email modal overrides with receipt content
    ...
    // Also call the onEmail callback if provided (for parent component handling)
    if (onEmail) {
      onEmail({
        subject: "Payment from Arcadia Academy of Music",
        content: emailContent,
  ...
  const handleSendEmail = React.useCallback(async (emailData: EmailFormData) => {
    if (!customerId) {
      toast.error("Customer ID is required to send email");
      return;
    }
  ...
  return (
    <>
      ...
      <EmailModal
        ...
        recipientEmails={customerEmail ? [customerEmail] : []}
        locationName={locationDetails?.name || "Arcadia Academy of Music"}
