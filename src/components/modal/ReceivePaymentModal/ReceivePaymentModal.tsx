 ...
 * 
 * This is a GLOBAL reusable component - always shows customer dropdown
 * Customer module uses its own modal, so this component is used in:
 * - Enrollment pages
 * - Future modules that need payment functionality
 * 
 * Always operates in dropdown mode - allows customer selection
 */
export const ReceivePaymentModal: React.FC<ReceivePaymentModalProps> = ({
  ...
  // Global component always uses dropdown mode (not fixed customer mode)
  // Customer module has its own modal, so this component never needs fixed mode
  const isInCustomerRoute: boolean = false;
  ...
  }, [state, calculations.amountToApply, calculatedAmountReceived, onSave]);
