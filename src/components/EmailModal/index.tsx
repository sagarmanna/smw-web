...
export function EmailModal({
  ...
  const generateTablesHTML = React.useCallback(() => {
...
<div style="margin-top:16px;">
  ...
  }, [privateLessonDueData, groupLessonDueData, invoiceData, creditData, totalBalance, hstNumber, locationName]);

  // Generate complete email HTML with introductory text AND tables
  const generateCompleteEmailHTML = React.useCallback(() => {
  ...
  // Initialize form when modal opens (only when transitioning from closed to open)
  React.useEffect(() => {
    ...
    if (open && !wasOpen) {
      ...
      // Pre-fill with recipient emails if available (don't set to empty if not available yet)
  ...
  // Update recipients when recipientEmails prop changes (e.g., after async API call completes)
  // This handles the case where emails are fetched asynchronously after modal opens
  React.useEffect(() => {
    if (open) {
      const validEmails = recipientEmails.filter(email => email && email.trim() !== "");
      if (validEmails.length > 0) {
        setRecipients(prev => {
          // If recipients is empty, set all valid emails
          // Otherwise, merge new emails with existing ones (avoiding duplicates)
          if (prev.length === 0) {
            return validEmails;
          }
          
          const currentEmailsSet = new Set(prev.map(e => e.toLowerCase()));
          const newEmails = validEmails.filter(email => !currentEmailsSet.has(email.toLowerCase()));
          
          if (newEmails.length > 0) {
            const combined = [...prev, ...newEmails];
            // Remove duplicates (case-insensitive)
            return Array.from(
              new Map(combined.map(email => [email.toLowerCase(), email])).values()
            );
          }
  ...
  const defaultLocalStorageKey = localStorageKey || `email-modal-${locationName}`;
...
export type { EmailFormData, EmailModalProps } from "./types";
