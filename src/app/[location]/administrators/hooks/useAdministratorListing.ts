...
import { AdministratorsQuery } from "../administrators.api";
import { SortField } from "../utils/sortAdministrators";

/**
 * Custom hook for managing administrators listing with server-side pagination, sorting, and filtering
 * 
 * @param location - The location slug for API calls
 * @returns Object containing administrators data, loading state, error state, and handlers for pagination, sorting, and filtering
 * 
 * @example
 * ```tsx
 * const { rows, total, isLoading, setPage, setSorting } = useAdministratorListing('maple');
 * ```
 */
export function useAdministratorListing(location: string) {
  ...
  const rows = useAppSelector((state) => state.administratorsListing.rows);
  const total = useAppSelector((state) => state.administratorsListing.total);
  const totalPages = useAppSelector((state) => state.administratorsListing.totalPages);
  const isLoading = useAppSelector((state) => state.administratorsListing.isLoading);
  const error = useAppSelector((state) => state.administratorsListing.error);
  const page = useAppSelector((state) => state.administratorsListing.page);
  const pageSize = useAppSelector((state) => state.administratorsListing.pageSize);
  const sortBy = useAppSelector((state) => state.administratorsListing.sortBy);
  const sortDir = useAppSelector((state) => state.administratorsListing.sortDir);
  const columnFilters = useAppSelector((state) => state.administratorsListing.columnFilters);
  const activeFilter = useAppSelector((state) => state.administratorsListing.activeFilter);
  ...
  const fetchData = React.useCallback(async () => {
    ...
    await dispatch(fetchAdministrators({ location, query }));
  ...
  React.useEffect(() => {
    ...
    if (!hasInitialFetchedRef.current || paramsChanged) {
      ...
      dispatch(fetchAdministrators({ location, query }));
    }
  ...
  const handleColumnFilterEnter = React.useCallback(() => {
    ...
    const query: AdministratorsQuery = {
