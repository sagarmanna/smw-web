...
interface CreatePhoneModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit?: (phone: AdministratorPhone) => void;
  editingPhone?: AdministratorPhone | null;
  location: string;
  administratorId: number;
  onUpdatePhones?: (phones: AdministratorPhone[]) => void;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = editingPhone
        ? await updateAdministratorPhone(
            location,
            administratorId,
            Number(editingPhone.id),
            phoneData
          )
        : await addAdministratorPhone(location, administratorId, phoneData);
      ...
      if (result?.success && result.data && Array.isArray(result.data)) {
        ...
        const transformedPhones: AdministratorPhone[] = result.data.map((item: {
          id: number;
          number: string;
