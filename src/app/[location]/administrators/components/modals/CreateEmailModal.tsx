...
interface CreateEmailModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit?: (email: AdministratorEmail) => void;
  editingEmail?: AdministratorEmail | null;
  location: string;
  administratorId: number;
  onUpdateEmails?: (emails: AdministratorEmail[]) => void;
  currentEmails?: AdministratorEmail[];
}
...
export function CreateEmailModal({
  ...
  const validateEmailWithAPI = async (value: string) => {
    try {
      ...
      const result = await validateAdministratorEmail(location, value);
      ...
      console.error("Error validating administrator email:", err);
    } finally {
  ...
  const handleEmailChange = (value: string) => {
    ...
    if (isEmailDuplicate(trimmed)) {
      setErrors({
        email:
          "This email is already used on this administrator record. Please use a different email.",
      });
      setIsValidatingEmail(false);
      return;
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = editingEmail
        ? await updateAdministratorEmail(
            location,
            administratorId,
        ...
        : await addAdministratorEmail(location, administratorId, baseData);
      ...
      if (result?.success && result.data) {
        ...
        const transformedEmails: AdministratorEmail[] = result.data.map(
          (item: {
            id: number;
            email: string;
            note?: string;
