...
interface CreateAddressModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit?: (address: AdministratorAddress) => void;
  editingAddress?: AdministratorAddress | null;
  location: string;
  administratorId: number;
  onUpdateAddresses?: (addresses: AdministratorAddress[]) => void;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = editingAddress
        ? await updateAdministratorAddress(
            location,
            administratorId,
            Number(editingAddress.id),
            addressData
          )
        : await addAdministratorAddress(location, administratorId, addressData);
      ...
      if (result?.success && result.data && Array.isArray(result.data)) {
        ...
        const transformedAddresses: AdministratorAddress[] = result.data.map((item: {
          id: number;
          address: string;
