...
const TAB_COMPONENTS: Record<string, React.ComponentType<{ location: string; staffMemberId: number }>> = {
  "history": HistoryTab,
};
    ...
    <div className="mt-8">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="inline-flex h-12 items-center justify-start rounded-md bg-muted p-1.5 text-muted-foreground w-full overflow-x-auto gap-1">
          {STAFF_MEMBER_TAB_ORDER.map((tabKey) => (
            <TabsTrigger
              key={tabKey}
              value={tabKey}
          ...
          return (
            <TabsContent key={tabKey} value={tabKey} className="mt-4">
              {isLoading && activeTab === tabKey ? (
                ...
                <TabComponent location={location} staffMemberId={staffMemberId} />
              )}
            </TabsContent>
          );
        })}
