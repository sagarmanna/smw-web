...
interface CreateEmailModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit?: (email: StaffMemberEmail) => void;
  editingEmail?: StaffMemberEmail | null;
  location: string;
  staffMemberId: number;
  onUpdateEmails?: (emails: StaffMemberEmail[]) => void;
  currentEmails?: StaffMemberEmail[];
}
...
export function CreateEmailModal({
  ...
  const validateEmailWithAPI = async (value: string) => {
    try {
      ...
      const result = await validateStaffMemberEmail(location, value);
      ...
      console.error("Error validating staff member email:", err);
    } finally {
  ...
  const handleEmailChange = (value: string) => {
    ...
    if (isEmailDuplicate(trimmed)) {
      setErrors({
        email:
          "This email is already used on this staff member record. Please use a different email.",
      });
      setIsValidatingEmail(false);
      return;
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = editingEmail
        ...
        : await addStaffMemberEmail(location, staffMemberId, baseData);
      ...
      if (result?.success && result.data) {
        ...
        const transformedEmails: StaffMemberEmail[] = result.data.map(
          (item: {
            id: number;
            email: string;
            note?: string;
