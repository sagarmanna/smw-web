...
interface CreateAddressModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit?: (address: StaffMemberAddress) => void;
  editingAddress?: StaffMemberAddress | null;
  location: string;
  staffMemberId: number;
  onUpdateAddresses?: (addresses: StaffMemberAddress[]) => void;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = editingAddress
        ? await updateStaffMemberAddress(
            location,
            staffMemberId,
            Number(editingAddress.id),
            addressData
          )
        : await addStaffMemberAddress(location, staffMemberId, addressData);
      ...
      if (result?.success && result.data && Array.isArray(result.data)) {
        ...
        const transformedAddresses: StaffMemberAddress[] = result.data.map((item: {
          id: number;
          address: string;
