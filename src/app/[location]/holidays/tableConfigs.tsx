...
import { ColumnDef, HeaderContext, Column } from "@tanstack/react-table";
import { ArrowUpDown } from "lucide-react";
import { HolidayRow } from "./holidays.api";

/**
 * Creates a sortable header component for table columns
 * 
 * @param title - The header title text
 * @param column - The table column instance
 * @returns React component for the sortable header
 */
function createSortableHeader(
  title: string,
  column: Column<HolidayRow, unknown>
) {
  ...
  return (
    <div
      ...
      onClick={handleClick}
    >
      <span className={canSort ? "cursor-pointer select-none" : ""}>
        {title}
      </span>
      {canSort && (
        <ArrowUpDown
...
export const holidaysColumns: ColumnDef<HolidayRow>[] = [
  {
    ...
    header: ({ column }: HeaderContext<HolidayRow, unknown>) =>
      createSortableHeader("Date", column),
  ...
  } as ColumnDef<HolidayRow>,
  {
    ...
    header: ({ column }: HeaderContext<HolidayRow, unknown>) =>
      createSortableHeader("Description", column),
    cell: ({ row }: { row: { original: HolidayRow } }) => (
