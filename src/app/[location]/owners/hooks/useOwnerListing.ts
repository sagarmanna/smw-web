...
import { 
  ...
  setColumnFilters, 
  setActiveFilter 
} from "../ownersListing.slice";
import { OwnersQuery } from "../owners.api";
import { SortField } from "../utils/sortOwners";

/**
 * Custom hook for managing owners listing with server-side pagination, sorting, and filtering
 * 
 * @param location - The location slug for API calls
 * @returns Object containing owners data, loading state, error state, and handlers for pagination, sorting, and filtering
 * 
 * @example
 * ```tsx
 * const { rows, total, isLoading, setPage, setSorting } = useOwnerListing('maple');
 * ```
 */
export function useOwnerListing(location: string) {
  ...
  const rows = useAppSelector((state) => state.ownersListing.rows);
  const total = useAppSelector((state) => state.ownersListing.total);
  const totalPages = useAppSelector((state) => state.ownersListing.totalPages);
  const isLoading = useAppSelector((state) => state.ownersListing.isLoading);
  const error = useAppSelector((state) => state.ownersListing.error);
  const page = useAppSelector((state) => state.ownersListing.page);
  const pageSize = useAppSelector((state) => state.ownersListing.pageSize);
  const sortBy = useAppSelector((state) => state.ownersListing.sortBy);
  const sortDir = useAppSelector((state) => state.ownersListing.sortDir);
  const columnFilters = useAppSelector((state) => state.ownersListing.columnFilters);
  const activeFilter = useAppSelector((state) => state.ownersListing.activeFilter);
  ...
  const fetchData = React.useCallback(async () => {
    ...
    await dispatch(fetchOwners({ location, query }));
  ...
  React.useEffect(() => {
    ...
    if (!hasInitialFetchedRef.current || paramsChanged) {
      ...
      dispatch(fetchOwners({ location, query }));
    }
