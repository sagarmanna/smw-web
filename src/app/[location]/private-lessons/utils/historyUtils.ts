/**
 * Utility functions for processing private lesson history messages
 */
...
/**
 * Processes placeholder patterns like {{privateLessonName}} to make them clickable
...
export function processHistoryPlaceholders(html: string): string {
  return html.replace(/\{\{([^}]+)\}\}/g, (_m, name: string) => {
    ...
    return `<a href="#" data-private-lesson-name="${safeName}" class="text-blue-600 hover:text-blue-800 font-medium underline">${safeName}</a>`;
  });
}
